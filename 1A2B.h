...
inline bool Get4NoRepeatNum(int iInputNums[4])
{
	...
	while (true)
	{
		...
		unsigned int uiUsedBitMask = 0;//用于验证是否重复使用数字
		for (int i = 0; i < 4; ++i)
		{
...
inline ValAB CalcAB(const int iRandNums[4], const int iInputNums[4])
{
	...
	for (int i = 0; i < 4; ++i)
	{
		uiRandBitMask |= (1 << iRandNums[i]);//构建位

		if (iRandNums[i] == iInputNums[i])//顺便计算A
		{
			++A;
		}
	}
	...
	for (int i = 0; i < 4; ++i)
	{
		if ((uiRandBitMask & (1 << iInputNums[i])) == (1 << iInputNums[i]))//测试位
		{
			++iCommon;
		}
	}
